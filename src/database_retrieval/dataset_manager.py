    ...
    def _filter_english_content_chunks(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'lang' in df.columns:
            ...
            if len(english_by_lang) > 0:
                ...
                print(f"   ⚠️ Language filtering found no matches, proceeding with content analysis...")
                english_by_lang = df  # Keep all for content analysis
        else:
            print(f"   ℹ️ No language column - analyzing all content")
            english_by_lang = df
        ...
        if 'chunk_text' in df.columns or 'text' in df.columns:
            ...
            print(f"   ⚠️ No text content column found for analysis!")
        ...
        if retention_rate < 20:
            print(f"   ⚠️ WARNING: Low retention rate ({retention_rate:.1f}%). Dataset may need manual inspection.")
